...
try {
    ...
    // искать в Общих настройках -> Адрес
    
    // LOGIN непосредственно с помощью него заходите в свой аккаунт,
    // в основном это ваша почта при регистрации
    // искать в Слева вверху аватар -> Профиль -> Email
    
    // HASH является api ключом для авторизации в сервисе,
    // искать в Слева вверху аватар -> Профиль -> API ключ
    // [Информация для чего нужен этот параметр](https://www.amocrm.ru/developers/content/api/auth)
